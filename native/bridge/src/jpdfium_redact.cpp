...
static int32_t objectFissionRedact(FPDF_DOCUMENT doc, FPDF_PAGE page, FPDF_TEXTPAGE textPage,
    ...
    // Set when an image cannot be promoted without hiding later form content.
    bool cropPromotionAmbiguous = false;
        ...
        for (int ci = childCount - 1; ci >= 0; ci--) {
            ...
            if (cropRect) {
                ...
                if (childType == FPDF_PAGEOBJ_IMAGE) {
                    ...
                    // form. Exact when the image is the first or last painted
                    // content of its form chain; if it is sandwiched, promoting
                    // would hide later siblings, so fail loudly instead.
    ...
    if (cropPromotionAmbiguous) return JPDFIUM_ERR_REDACT_INCOMPLETE;
    ...
    auto emitFragment = [&](FPDF_PAGEOBJECT obj, const TextFragment& frag, const FissionPlan& plan,
        ...
        if ((!strategyAUsed || !verifyWithTextPage) && emissionStatus == -1 &&
            !isStandard14Font(plan.font) && frag.hasExpectedBox) {
    ...
    for (auto& plan : plans) {
        ...
        if (plan.fragments.size() == 1) {
            ...
            if (canEdit) {
                ...
                if (mutated) objsToDestroy.insert(plan.originalObj);
            }
