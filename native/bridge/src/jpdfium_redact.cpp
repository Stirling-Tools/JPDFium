...
static FS_RECTF makePageRect(float l, float b, float r, float t) {
    FS_RECTF out{};
    out.left = l;
    out.top = t;
    out.right = r;
...
// Invert an affine matrix (row-vector convention: p' = p * L + t). Returns
// false for a degenerate linear part.
static bool invertMatrix(const FS_MATRIX& m, FS_MATRIX& out) {
    const double det = static_cast<double>(m.a) * m.d - static_cast<double>(m.b) * m.c;
    if (std::abs(det) < 1e-12) return false;
    out.a = static_cast<float>(m.d / det);
    out.b = static_cast<float>(-m.b / det);
    out.c = static_cast<float>(-m.c / det);
    out.d = static_cast<float>(m.a / det);
    out.e =
        static_cast<float>((static_cast<double>(m.c) * m.f - static_cast<double>(m.d) * m.e) / det);
    out.f =
        static_cast<float>((static_cast<double>(m.b) * m.e - static_cast<double>(m.a) * m.f) / det);
    return true;
}
...
static int32_t objectFissionRedact(FPDF_DOCUMENT doc, FPDF_PAGE page, FPDF_TEXTPAGE textPage,
                                   ...
                                   const FS_RECTF* cropRect = nullptr) {
...
#ifdef JPDFIUM_HAS_FREETYPE
    struct FtFontCache {
        FT_Face face = nullptr;  // owned; freed after the fission plans are built
    ...
    auto markFormContents = [&](auto& self, FPDF_PAGEOBJECT formObj, const FS_MATRIX& parentToPage,
        ...
        for (int ci = childCount - 1; ci >= 0; ci--) {
            ...
            if (childType == FPDF_PAGEOBJ_FORM) {
                FS_MATRIX childMatrix;
                if (FPDFPageObj_GetMatrix(child, &childMatrix)) {
                    // Never bound-test a form (FPDFPageObj_GetBounds can be
                    // degenerate for forms); its children decide. Matrix order
                    // matches CPDF_TextPage::ProcessFormObject.
            ...
            if (cropRect) {
                ...
                if (childType == FPDF_PAGEOBJ_IMAGE) {
                    ...
                    int targetIdx = ref.topFormPageIndex + 1;
    ...
    for (auto& plan : plans) {
        ...
        // One surviving run edits the original object in place. A new object
        // would land in a stream appended after all existing content, so it
        // would paint above later objects (and above its form).
        if (plan.fragments.size() == 1) {
            ...
            bool canEdit = true;
            if (plan.parentForm) {
                // frag.matrix is page space; a form child's matrix is form-local.
                auto pit = objPtrToIndex.find(reinterpret_cast<uintptr_t>(plan.originalObj));
                if (pit == objPtrToIndex.end() ||
                    !invertMatrix(allObjs[pit->second].toPage, invChain)) {
                    canEdit = false;
                } else {
                    matrix = concatMatrix(frag.matrix, invChain);
                    invChainPtr = &invChain;
                }
            }
            if (canEdit) {
                // Snapshot text and matrix so a failed in-place attempt can be
                // undone (the stale text page yields the pre-edit text).
                std::vector<FPDF_WCHAR> originalText;
                unsigned long needBytes =
                    FPDFTextObj_GetText(plan.originalObj, textPage, nullptr, 0);
                if (needBytes > sizeof(FPDF_WCHAR)) {
                    originalText.resize(needBytes / sizeof(FPDF_WCHAR));
                    if (FPDFTextObj_GetText(plan.originalObj, textPage, originalText.data(),
                                            needBytes) != needBytes) {
                        originalText.clear();
                    }
                }
                FS_MATRIX originalMatrix;
                bool haveOriginalMatrix =
                    FPDFPageObj_GetMatrix(plan.originalObj, &originalMatrix) != 0;
                bool mutated = false;
                ...
                if (mutated) {
                    // Never ship a partially rewritten text object.
                    bool restored =
                        !originalText.empty() &&
                        FPDFText_SetText(plan.originalObj,
                                         reinterpret_cast<FPDF_WIDESTRING>(originalText.data()));
                    if (haveOriginalMatrix) {
                        FPDFPageObj_SetMatrix(plan.originalObj, &originalMatrix);
                    }
                    if (!restored) objsToDestroy.insert(plan.originalObj);
                }
            }
