...
public final class PdfDocument implements AutoCloseable {
    ...
    /**
     * Append all pages from another document to the end of this document.
     *
     * @param other document to import pages from
     */
    public void appendDocument(PdfDocument other) {
        ensureOpen();
        if (other == null) throw new IllegalArgumentException("other document must not be null");
        other.ensureOpen();
        insertDocument(pageCount(), other);
    }

    /**
     * Insert all pages from another document into this document at the specified page index.
     *
     * @param atPageIndex zero-based index where pages should be inserted
     * @param other       document to import pages from
     */
    public void insertDocument(int atPageIndex, PdfDocument other) {
        ensureOpen();
        if (other == null) throw new IllegalArgumentException("other document must not be null");
        other.ensureOpen();
        if (atPageIndex < 0 || atPageIndex > pageCount()) {
            throw new IndexOutOfBoundsException("Invalid insertion index: " + atPageIndex);
        }
        if (other.pageCount() == 0) return;
        PdfPageImporter.importPages(rawHandle(), other.rawHandle(), null, atPageIndex);
    }
